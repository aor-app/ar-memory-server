...
class CommandErr extends Error {
...
class ARReceiver {
    ...
    static async build(){
        ...
        let portList = ['/dev/receiver', '/dev/ttyACM0'];
    ...
    _writeCommand (data, waitTime, receiveResultCount)  {
        ...
        return new Promise((resolve, reject) => {
            try{
                ...
                setTimeout(() => {
                    ...
                    this.port.write(commandSJIS, (err, results) => {
                        let receiveCount = 0;
                        let retArray = new Array;
                        let receiveBuf = (input) => {
                            let buffer = Buffer.from(input, 'binary');
                            let retStr = iconv.decode(buffer, 'Shift_JIS');
                            receiveCount += 1;
                            try {
                                console.log(`${data}: ${retStr}`);
                        ...
                        if ( receiveResultCount != 1 ){
                            console.log('count 15s.');
                            setTimeout(() => {
                                if (receiveCount !== receiveResultCount ){
                                    reject({
                                        code: -2,
    ...
    async _executeWrite (str) {
        ...
        if(resultCode != '20'){
            //        throw new CommandErr(-2, "command execute error.");
        }
    ...
    async writeMemory() {
        try {
            ...
            if (!resultBuf) {
                throw new CommandErr(-3, 'write memory faild.');
            }
    ...
    getModel () {
        ...
        return new Promise((resolve, reject) => {
            if ( this.machineStatus != null ) {
                reject(new CommandError( -1, 'busy.'));
            }else{
...
module.exports.CommandError = CommandErr;
